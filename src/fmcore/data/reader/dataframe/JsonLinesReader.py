...
from fmcore.constants import DataLayout, FileFormat, MLTypeSchema, Storage
...
from fmcore.util.aws.s3 import S3Util
...
class JsonLinesReader(DataFrameReader):
    ...
    def _read_raw_sdf(
        ...
        **kwargs,
        ...
        if (
            read_as is DataLayout.LIST_OF_DICT
            and self.params.orient == "records"
            and self.params.lines is True
        ):
    ...
    @staticmethod
    def read_list_of_dict(
        source: Union[str, io.IOBase], storage: Optional[Storage] = None
    ) -> Optional[List[Dict]]:
        if storage is None:
