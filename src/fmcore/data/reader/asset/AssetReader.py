...
from fmcore.util import Log, String, accumulate, as_list, dispatch, is_list_like, retry
...
class AssetReader(Reader, ABC):
    ...
    def _read_stream(
        ...
        **kwargs,
        ...
        return self._verify_asset(
            self._read_asset_with_retries(source=stream, storage=Storage.STREAM, **kwargs)
        )
    ...
    def _read_url(
        ...
        **kwargs,
        ...
        return self._verify_asset(self._read_asset_with_retries(source=url, storage=Storage.URL, **kwargs))
        ...
        if is_list_like(local_path):
            return [
                ...
                for asset in self._read_asset_multi(
                    source=local_path, storage=Storage.LOCAL_FILE_SYSTEM, **kwargs
                )
            ]
        return self._verify_asset(
            self._read_asset_with_retries(source=local_path, storage=Storage.LOCAL_FILE_SYSTEM, **kwargs)
        )
    ...
    def _read_s3(
        ...
        **kwargs,
        ...
        if is_list_like(s3_path):
            return [
                self._verify_asset(asset)
                for asset in self._read_asset_multi(source=s3_path, storage=Storage.S3, **kwargs)
            ]
        return self._verify_asset(self._read_asset_with_retries(source=s3_path, storage=Storage.S3, **kwargs))

    def _read_asset_multi(
        self, source: List[str], storage: Storage, file_contents: Optional[FileContents] = None, **kwargs
    ) -> List[Asset]:
        ...
        if len(failed_read_file_paths) > 0:
            raise IOError(
                f"Could not read assets from the following paths:\n{sorted(list(failed_read_file_paths))}"
            )
        return assets
