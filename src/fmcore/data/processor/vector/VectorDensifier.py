...
from fmcore.data.processor import SingleColumnProcessor, SparseVectorInputProcessor, VectorOutputProcessor
...
class VectorDensifier(SingleColumnProcessor, SparseVectorInputProcessor, VectorOutputProcessor):
    ...
    def transform_single(self, data: SparseCSRMatrix) -> Optional[np.ndarray]:
        ...
        if len(data.shape) != 2:
            raise ValueError(
                f"Each SparseCSRMatrix to densify must have two dimensions. Found: {len(data.shape)} dims"
            )
        if data.shape[0] != 1:
            raise ValueError(
                f"Each SparseCSRMatrix to densify must have exactly 1 row. Found: {data.shape[0]} rows"
            )
