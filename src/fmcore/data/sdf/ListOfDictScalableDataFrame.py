...
from fmcore.data.sdf import RAW_DATA_MEMBER, ScalableDataFrame
...
class ListOfDictScalableDataFrame(ScalableDataFrame):
    ...
    def __getitem__(self, key) -> Union[Any, ListOfDictScalableDataFrame, NumpyArrayScalableSeries]:
        ...
        if isinstance(key, np.ndarray):
            ...
            if np.issubdtype(key.dtype, int):
                ...
                return self._constructor(
                    [self._data[i] for i, should_select in enumerate(key) if should_select], name=self.name
                )
            raise TypeError(
        ...
        if isinstance(key, str):
            ...
            col = key
            out_vals = [d[col] for d in self._data]
        ...
        if isinstance(key, (list, set)):
            ...
            if len(missing_cols) > 0:
                raise KeyError(f"Following columns could not be found in Dataframe: {missing_cols}")
            return self._constructor(
                [{k: v for k, v in d.items() if k in key} for d in self._data], name=self.name
            )
        ...
        if isinstance(key, tuple):
            ...
            if isinstance(cols, slice):
                if any_are_not_none(cols.start, cols.stop, cols.step):
                    raise IndexError(
                        "Second argument when indexing (i.e. columns) should be a list or an empty slice"
                    )
                return out_df
            else:
                return out_df[cols]
    ...
    def __setitem__(self, key: Any, value: Any):
        ...
        if isinstance(key, str):
            if isinstance(value, ScalableSeries):
                if length != len(value):
                    raise ValueError(
                        f"Expected input {type(value)} to have length={length}; found length={len(value)}"
                    )
                ...
                if length != len(value):
                    raise ValueError(
                        f"Expected input value to have length={length}; found length={len(value)}"
                    )
                for i, val in enumerate(value):
                    self._data[i][key] = val
    ...
    def apply(self, func, axis=1, args=(), **kwargs):
        ...
        out: List[Any] = [func(d, *args, **kwargs) for d in self._data]
    ...
    @classmethod
    def _concat_sdfs(
        cls, sdfs: List[ListOfDictScalableDataFrame], reset_index: bool
    ) -> ListOfDictScalableDataFrame:
    ...
    def as_numpy(self, sort_columns: bool = True, **kwargs) -> np.recarray:
        ...
        np_dtype = np.dtype([(col, object) for col in cols])
        return np.rec.array([tuple([d.get(col, np.nan) for col in cols]) for d in self._data], dtype=np_dtype)
    ...
    def applymap(self, func, na_action: Optional[Literal["ignore"]] = None, **kwargs) -> ScalableDataFrame:
        ...
        return self._constructor([{k: mapper(v) for k, v in d.items()} for d in self._data], name=self.name)
