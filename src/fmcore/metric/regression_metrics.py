...
from fmcore.framework import Metric, RegressionPredictions
...
class MeanSquaredError(Metric):
    ...
    def update(self, data: RegressionPredictions):
        ...
        self._sum_of_sq_diff: Decimal = Decimal(
            math.fsum(
                [
    ...
    def compute(self) -> confloat(ge=0.0, le=1.0):
        ...
        metric_val: float = float(Fraction(self._sum_of_sq_diff) / Fraction(self._num_total_examples))
        return metric_val
...
class MeanAbsoluteError(Metric):
    ...
    def update(self, data: RegressionPredictions):
        ...
        self._num_total_examples += len(data.ground_truth_scores)
        ## Ref: https://stackoverflow.com/a/59128677
        self._sum_of_abs_diff: Decimal = Decimal(
            math.fsum(
                [
                    self._sum_of_abs_diff,
                    Decimal((data.ground_truth_scores - data.predicted_scores).abs().sum()),
                ]
            )
        )
    ...
    def compute(self) -> confloat(ge=0.0, le=1.0):
        ## Ref: https://www.laac.dev/blog/float-vs-decimal-python/
        metric_val: float = float(Fraction(self._sum_of_abs_diff) / Fraction(self._num_total_examples))
        return metric_val
