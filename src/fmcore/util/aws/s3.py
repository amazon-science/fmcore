...
from fmcore.util import FileSystemUtil, Log, String, Timer, Utility, shuffle_items
from fmcore.util.language import any_are_none, as_list, remove_values
...
class S3Util(Utility):
    ...
    def s3_path_exploder_dict(cls, s3_path: str) -> Dict[str, str]:
        ...
        return {cls.S3_BUCKET: s3_bucket, cls.OBJECT_KEY: object_key}
    ...
    def check_bucket_permission(cls, s3_path: str, action_names: Union[str, List[str]]) -> bool:
        ...
        results = iam.simulate_principal_policy(
            PolicySourceArn=arn, ResourceArns=[bucket_objects_arn], ActionNames=action_names
        )
    ...
    def can_read_bucket(cls, s3_path: str) -> bool:
        return cls.check_bucket_permission(
            s3_path, [cls.S3_BUCKET_GET_OBJ_PERMISSION, cls.S3_BUCKET_LIST_PERMISSION]
        )
    ...
    def get_s3_object_size(
        ...
        s3_obj_details: List = [cls.get_s3_object_details(s3_fpath) for s3_fpath in as_list(s3_path)]
        if any_are_none(*s3_obj_details) and not ignore_missing:
            return None
        size_in_bytes: int = int(
            sum([s3_resp["ContentLength"] for s3_resp in s3_obj_details if s3_resp is not None])
        )
        if unit is not None:
        ...
        *,
        ...
        for resp in paginator.paginate(Bucket=s3_bucket_resource.name, **pagination_params):
            if "CommonPrefixes" in resp:
                subdirs.extend(
                    [
                        os.path.join(String.S3_PREFIX, s3_bucket, f["Prefix"])
                        if not names_only
        ...
        for retry_wait_time in retry_wait_times:
            try:
                ...
                if retry_wait_time != retry_wait_times[-1]:
                    Log.debug(
                        f'Retrying retrieval of object at bucket "{s3_bucket}" with key "{object_key}"...'
                    )
        raise IOError(
            f'Cannot retrieve S3 object after {retry} attempts; bucket="{s3_bucket}", key="{object_key}"'
        )
    ...
    def put_s3_object_str(
        ...
        raise IOError(
            "Could not successfully put object at bucket %s and key %s, after %s attempts"
    ...
    def copy_local_file_to_s3(cls, source_local_path: str, destination_s3_path: str, extra_args=None) -> bool:
        ...
        try:
            ...
            s3.meta.client.upload_file(
                source_local_path, destination_s3_bucket, destination_object_key, extra_args
            )
            assert cls.s3_object_exists(destination_s3_path), (
                f"Could not find file {destination_s3_path} after copying"
            )
            return True
        except Exception as e:
            Log.error(String.format_exception_msg(e))
    ...
    def copy_local_dir_to_s3(
        ...
        for local_fpath in shuffle_items(local_fpaths):
            ...
            try:
                ...
                if log:
                    Log.info(
                        f'...uploaded file from "{local_fpath}" to "{s3_fpath}" in {timer.time_taken_str}.'
                    )
            except Exception as e:
                Log.error(String.format_exception_msg(e))
                return False
    ...
    def copy_s3_file_to_local(cls, source_s3_path: str, destination_local_path: str, extra_args=None) -> bool:
        ...
        try:
            ...
            s3.meta.client.download_file(
                source_s3_bucket, source_object_key, destination_local_path, extra_args
            )
    ...
    def copy_s3_dir_to_local(
        ...
        s3_fnames: Set[str] = {s3_fpath.split(String.SLASH)[-1] for s3_fpath in s3_fpaths}
        ...
        if force_download:
            ...
            local_fnames: Set[str] = {local_fpath.split(os.path.sep)[-1] for local_fpath in local_fpaths}
        if local_fnames < s3_fnames:
            ...
            if len(s3_fpaths_to_download) == 0:
                return True
            time.sleep(
                random.randint(0, 30000) / 1000
            )  ## Wait randomly between 0 and 30 seconds to acquire locks.
            ...
            if not FileSystemUtil.file_exists(lock_file):
                ...
                try:
                    ...
                    for s3_fpath_to_download in s3_fpaths_to_download:
                        ...
                        with Timer(silent=True) as timer:
                            cls.copy_s3_file_to_local(
                                s3_fpath_to_download, local_fpath, extra_args=extra_args
                            )
                        if log:
                            Log.info(
    ...
    @classmethod
    def copy_file_between_s3_locations(
        cls, source_s3_path: str, destination_s3_path: str, extra_args=None
    ) -> bool:
        if extra_args is None:
            extra_args = {}
        try:
    ...
    @classmethod
    def copy_dir_between_s3_locations(
        cls, source_s3_dir: str, destination_s3_dir: str, extra_args=None
    ) -> bool:
        ...
        try:
            for source_s3_fpath in source_s3_fpaths:
                ...
                cls.copy_file_between_s3_locations(
                    source_s3_fpath, destination_s3_fpath, extra_args=extra_args
                )
            return True
        except Exception as e:
            Log.error(String.format_exception_msg(e))
