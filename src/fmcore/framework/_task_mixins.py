...
from bears.core.frame import (
    ScalableDataFrame,
    ScalableDataFrameOrRaw,
    ScalableOrRaw,
    ScalableSeries,
    ScalableSeriesOrRaw,
    TensorScalableSeries,
)
from bears.reader import AssetReader, DataFrameReader
...
from bears.util.language._import import _IS_TORCH_INSTALLED
...
class InputOutputDataMixin(TaskRegistryMixin, ABC):
    ...
    def _set_data_params(cls, params: Dict) -> Dict:
        ...
        if params.get("data") is None:
            raise ValueError(f"Missing param: `data`. Please pass this param to {cls.class_name}.of(...)")
        ...
        if params.get("data_schema") is None:
            raise ValueError(
                f"Missing param: `data_schema`. Please pass this param to {cls.class_name}.of(...)"
            )
        return params

    ## Runs after any similar validators on subclasses(Dataset, Predictions, etc).
    @model_validator(mode="before")
    @classmethod
    ...
    def to_string(self, *, data_schema: bool = True) -> str:
        if self.in_memory():
            out: str = (
                f"{str(self.data_split).capitalize() if self.data_split is not None else ''}"
                f" {self.class_name}"
    ...
    def _registry_keys(cls) -> Optional[Union[List[Any], Any]]:
        ...
        return tasks + [(task, String.str_normalize(cls.class_name)) for task in tasks]
    ...
    def to_layout(self, layout: DataLayout, **kwargs) -> Any:
        return self.update_params(data=self.data.to_layout(layout=layout, **kwargs))
    ...
    def copy(self, deep: bool = False) -> Any:
        ...
        if deep is not False:
            raise ValueError("We can only perform shallow copies.")
        return self.update_params()  ## With empty overrides, this just copies the Parameters object.

    def torch_dataloader(
        self, *, error: Literal["raise", "warn", "ignore"] = "raise", **kwargs
    ) -> Optional[Any]:
        if _IS_TORCH_INSTALLED:
            from fmcore.framework.dl.torch import PyTorchTaskDataset

            return PyTorchTaskDataset(dataset=self, **kwargs).dataloader()
        return None
    ...
    def read(
        ...
        **kwargs,
        ...
        if not self.in_memory():
            ...
            kwargs["raw"] = False
            data: ScalableDataFrame = reader.read_metadata(file_metadata, read_as=read_as, **kwargs)
            if persist:
        ...
        if map_apply != "none":
            ...
            dataset: InputOutputDataMixin = dataset.map(
                fn=map, apply=map_apply, validate=validate, **map_kwargs
            )
        return dataset
    ...
    def iter(
        ...
        **kwargs,
        ...
        if steps is not None and batch_size is None:
            raise ValueError("Cannot have empty `batch_size` when passing `steps`.")
        ...
        **map_kwargs,
        ...
        if map_apply != "none":
            ...
            data_batch: InputOutputDataMixin = data_batch.map(
                fn=map, apply=map_apply, validate=should_validate, **map_kwargs
            )
    ...
    def iter_files(self, **kwargs) -> Generator[InputOutputDataMixin, None, None]:
        if self.in_memory():
            raise ValueError("Cannot iterate over files for data which is in memory")
    ...
    def read_assets(
        ...
        **kwargs,
        ...
        tensor_layout: Optional[DataLayout] = SHORTHAND_TO_TENSOR_LAYOUT_MAP.get(
            String.str_normalize(tensor_type), None
        )
            ...
            for col, col_mltype in self.columns(mltypes=asset_mltype, return_mltypes=True):
                ...
                if col not in asset_readers:
                    ...
                    if len(asset_formats) == 0:
                        raise ValueError(
                            f"Cannot detect any file formats from asset paths: {asset_paths.to_list()}"
                        )
                    ...
                    asset_readers[col] = AssetReader.of(file_format=asset_format, **kwargs)
        ...
        for col in assets.keys():
            self.data[col] = assets[col].apply(
                self._accumulate_and_convert_asset, tensor_layout=tensor_layout
            )
        if return_cols:
    ...
    def map(
        ...
        **kwargs,
        ...
        mapped_data_batch: InputOutputDataMixin = self._map_data(
            mapped_data_batch, fn=fn, apply=apply, **kwargs
        )
    ...
    def _map_data(
        ...
        **kwargs,
        ...
        if fn is None:
            return data_batch
        if apply is None:
            raise ValueError("Must pass value for `apply` when calling .map()")
        if apply == "batch":
            ...
            mapped_data: ScalableDataFrame = ScalableDataFrame.of(
                data_batch.data.apply(fn, args=(data_schema,))
            )
            out: InputOutputDataMixin = data_batch.of(
                data=mapped_data,
                data_schema=data_schema,
        ...
        **kwargs,
        ...
        if mltypes is not None:
            data_schema: MLTypeSchema = Schema.filter_schema(
                data_schema=data_schema, mltypes=mltypes, **kwargs
            )
            if return_mltypes:
                return sorted([(col, mltype) for col, mltype in data_schema.items()], key=lambda x: x[0])
            else:
    ...
    def filter(
        ...
        **kwargs,
        ...
        for batch in self.iter(**kwargs):
            ...
            if (rows_to_keep == False).all():  # noqa
                continue
    ...
    def __getattr__(self, attr_name: str):
        ...
        raise AttributeError(
            f"'{attr_name}' is neither an attribute of {self.class_name} nor a data column; "
    ...
    def __getitem__(self, attr_name: str):
        if attr_name in self.data.columns_set:
            return self.data[attr_name]
        raise KeyError(f"'{attr_name}' is not a data column; current data columns are: {self.data.columns}")
    ...
    def features(
        ...
        **kwargs,
        ...
        return Schema.filter_df(
            self.data, data_schema=features_schema, return_series=return_series, sort_columns=True, **kwargs
        )
    ...
    def ground_truths(
        self, *, allow_missing: bool = False, return_series: bool = True, return_index: bool = False, **kwargs
        ...
        return Schema.filter_df(
            self.data, cols, allow_missing=allow_missing, return_series=return_series, **kwargs
        )
    ...
    def in_memory(self, raise_error: bool = False) -> bool:
        error_msg: str = (
            "Cannot use data which is not in memory, "
            "please read it into memory using .read(...) or .iter(...)"
        )
