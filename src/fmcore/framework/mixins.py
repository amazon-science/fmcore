...
class InputOutputDataMixin(TaskRegistryMixin, ABC):
    ...
    @root_validator(
        pre=False
    )  ## Runs after any @root_validator(pre=True) on Dataset and Predictions subclasses.
    ...
    def torch_dataloader(
        ...
        with optional_dependency("torch", error=error):
            from fmcore.framework.dl.torch import PyTorchTaskDataDataset
    ...
    def __filter_yield(
        self, fn: Union[Callable[[Dict, Schema], bool], Callable[[Dict], bool]], **kwargs
    ...
    def __getitem__(self, attr_name: str):
        if attr_name in self.data.columns_set:
            return self.data[attr_name]
        raise KeyError(f"`{attr_name}` is not a data colum; current data columns are: {self.data.columns}")
