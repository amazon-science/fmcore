...
from typing import Any, Callable, ClassVar, Dict, List, Literal, Optional, Set, Tuple, Union

import numpy as np
import pandas as pd
from bears import FileMetadata, ScalableDataFrame, ScalableSeries
...
from fmcore.framework._task.classification import ClassificationData, Classifier, MultiLabelClassifier
from fmcore.framework._task.retrieval import Queries, RankedResults, Retriever
...
class ICLSampler(Parameters):
    ...
    def set_icl_sampler_params(cls, params: Dict) -> Dict:
        ...
        set_param_from_alias(
            params, param="num_shots", alias=["icl_num_shots", "n_shots", "shots", "icl_shots"]
        )
    ...
    def icl_expanded_prompt_template(self) -> str:
        ...
        for shot_idx in self.shot_idxs(self.num_shots):
            ...
            num_shots_icl_template += self.icl_template.replace(
                f"{ICL_EXAMPLE_TEMPLATE_KEY}[", f"{ICL_EXAMPLE_TEMPLATE_KEY}_{shot_idx}["
            )
        ...
        prompt_template: str = to_second_level_prompt_template(
            prompt_template, exclude=ICL_EXAMPLES_TEMPLATE_KEY
        )
    ...
    def _select_shot_i_icl_examples(
        ...
        for batch_i in range(len(batch_icl_idxs_in_use)):
            ...
            if icl_filter_col is not None:
                ...
                icl_df_filtered: pd.DataFrame = icl_df_filtered[
                    icl_df_filtered[icl_filter_col] == batch_filter_col_val
                ]
            ...
            try:
                if balance_on is not None:
                    ...
                    icl_example: Dict = (
                        icl_df_filtered.sample(frac=1)
                        .drop_duplicates(subset=[balance_on])
                        .sample(frac=1)
                        .iloc[0]
                        .to_dict()
                    )
...
class ClassificationICLSampler(ICLSampler):
    ...
    def set_classification_icl_sampler_params(cls, params: Dict) -> Dict:
        ...
        icl_dataset.data[icl_gt_lb_col] = (
            icl_dataset.data[icl_gt_lb_col].apply(params["label_normalizer"]).map(params["label_verbalizer"])
        )
        ...
        if params["strategy"] == "entailment":
            ...
            for normalized_lb in icl_dataset.data.pandas()[icl_gt_lb_col].unique():
                icl_entailment_pos_dfs[normalized_lb] = set_col_val(
                    icl_dataset.data.pandas()
                    .query(f'`{icl_gt_lb_col}` == "{normalized_lb}"')
                    .copy(deep=True),
            ...
            icl_entailment_pos: pd.DataFrame = pd.concat(list(icl_entailment_pos_dfs.values())).reset_index(
                drop=True
            )
            params["icl_entailment_pos_dataset"]: ClassificationData = icl_dataset.update_params(
                data=icl_entailment_pos
            )
            icl_entailment_neg: pd.DataFrame = pd.concat(list(icl_entailment_neg_dfs.values())).reset_index(
                drop=True
            )
    ...
    def _append_icl_examples_entailment(self, batch: ClassificationData) -> ClassificationData:
        ...
        for shot_i in self.shot_idxs(self.num_shots):  ## 1, 2, 3, ...
            ...
            if self.label_sampling_strategy == "representative":
                ...
                raise NotImplementedError(
                    f'Unsupported `label_sampling_strategy`: "{self.label_sampling_strategy}"'
                )
...
def apply_prompt_template(batch: "Prompts", prompt_prefix: str) -> "Prompts":
    if is_dict_like(batch.prompt_template):
        if batch.prompt_template_apply == "expand":
            error_msg: str = (
                f'When passing prompt template as a dict and `prompt_template_apply`="expand", '
            ...
            for data_d in batch.data.to_list_of_dict():
                for prompt_template_id, prompt_template in prompt_templates.items():
                    ...
                    d[TEXT_PROMPT_COL]: str = _create_prompt(
                        prompt_template, prompt_prefix=prompt_prefix, **d
                    )
            ...
            error_msg: str = (
                f'When passing prompt template as a dict and `prompt_template_apply`="map", '
            ...
            for d in batch.data.to_list_of_dict():
                ...
                if prompt_template_map_val is None:
                    raise ValueError(
                        error_msg
                        + f'Found `None` value in column "{batch.prompt_template_map_col}", which cannot be '
                ...
                if prompt_template is None:
                    raise ValueError(
                        error_msg
                        + f"Found no corresponding template for value {d[batch.prompt_template_map_col]} in "
        ...
        if batch.prompt_template_apply != "expand":
            raise ValueError(
                'When `prompt_template` is not a dict, `prompt_template_apply`="expand" must be passed; we will '
                "either apply the prompt template to each row (if it is a string) or expand each row along the prompt "
                "template (if it is list-like)."
            )
    ...
    batch.data_schema.features_schema[f"{PROMPT_TEMPLATE_INDEX_COL_PREFIX}{batch.data_schema.index_col}"] = (
        MLType.CATEGORICAL
    )
    return batch


def to_second_level_prompt_template(
    prompt_template: str, *, exclude: Optional[Union[Set[str], str]] = None
) -> str:
...
class Prompts(Dataset):
    ...
    def set_prompt_data_params(cls, params: Dict) -> Dict:
        set_param_from_alias(
            params, param="prompt_template", alias=["template", "prompt", "prompt_templates"]
        )
        ...
        if (
            is_dict_like(params["prompt_template"])
            and params["prompt_template_apply"] == "map"
            and not isinstance(params.get("prompt_template_map_col"), str)
        ):
            raise ValueError(
                'When passing `prompt_template` as a dict and `prompt_template_apply`="map" (default), '
                "you must also pass `prompt_template_map_col`, which should be the column you want to map "
                "based on the prompts dict."
            )
        return params
...
class TextGenerationParams(Parameters, ABC):  ## Becomes an anonymous class later
    ...
    def set_gen_params(cls, params: Dict) -> Dict:
        ...
        if params["output_scores_format"] == "probabilities":
            ...
            raise NotImplementedError(
                f'Unsupported `output_scores_format`: "{params["output_scores_format"]}"'
            )
...
class TextGenerationParamsMapper(MappedParameters):
    mapping_dict: ClassVar[Dict[Tuple[str, ...], TextGenerationParams]] = {
...
class GenerativeLM(Algorithm, ABC):
    ...
    def _create_predictions(self, batch: Prompts, predictions: Dict, **kwargs) -> NextTokens:
        ...
        if (
            "generated_token_ids" in predictions
...
class LanguageModelTaskMixin(Algorithm, ABC):
    ...
    class Hyperparameters(Algorithm.Hyperparameters):
        ...
        def set_lm_task_params(cls, params: Dict) -> Dict:
            set_param_from_alias(
                params, param="lm", alias=["llm", "prompter", "base_llm", "base_model", "base"]
            )
    ...
    def initialize(self, model_dir: Optional[FileMetadata] = None):
        if self.icl_dataset is not None:
            if self.hyperparams.icl_template is None:
                raise ValueError(
                    "When passing an In-Context Learning dataset, you must also pass hyperparameter "
                    "`icl_template`"
                )
        ...
        if self.lm is None and self.hyperparams.lm is None:
            ...
            self.lm: GenerativeLM = GenerativeLM.of(
                **{
                    **dict(model_dir=model_dir, task=Task.NEXT_TOKEN_PREDICTION),
                    **self.hyperparams.lm,
                    **dict(
    ...
    def _lm_predict(self, prompts: Prompts, **kwargs) -> NextTokens:
        ...
        if not isinstance(next_tokens, NextTokens):
            raise ValueError(
                f"Expected language model output to be {NextTokens}; found: {type_str(next_tokens)}"
            )
        return next_tokens
...
class FewShotRetrievalAugmentedTextGenerator(LanguageModelTaskMixin):
    ...
    def predict_step(self, batch: TextInputs, query_template: Optional[str] = None, **kwargs) -> Any:
        ...
        if query_template is None:
            raise ValueError(
                "`query_template` must be passed either as a hyperparam or as a keyword argument during prediction."
            )
    ...
    def _retriever_predict(self, queries: Queries, **kwargs) -> RankedResults:
        ...
        if isinstance(self.retriever, Evaluator):
            retriever_batch_size: int = get_default(
                self.retriever._create_hyperparams().batch_size, self.hyperparams.batch_size
            )
        ...
        if not isinstance(ranked_results, RankedResults):
            raise ValueError(
                f"Expected retriever output to be {RankedResults}; found: {type_str(ranked_results)}"
            )
...
class FewShotTextClassifier(LanguageModelTaskMixin, Classifier):
    ...
    class Hyperparameters(LanguageModelTaskMixin.Hyperparameters):
        ...
        def set_zeroshot_params(cls, params: Dict) -> Dict:
            ...
            if not isinstance(params["prompt_template"], str):
                raise ValueError(
                    f"Expected prompt template to be a string: found {type(params['prompt_template'])}"
                )
            ...
            if not is_dict_like(lm_hyperparams):
                raise ValueError(
                    f"Expected `lm` to have dict-like `hyperparams`; found {type(lm_hyperparams)}"
                )
            ...
            lm_hyperparams["generation_params"][
                "output_scores_format"
            ]: GenerationOutputScoresFormat = "logits"
    ...
    def zeroshot_entailment(
        ...
        *,
        ...
        row[ENTAILMENT_PROBABILITY_COL], row[CONTRADICTION_PROBABILITY_COL] = stable_softmax(
            yes_score, no_score
        )
        return row
    ...
    def calculate_token_score(
        ...
        *,
        ...
        for token_timestep_i in token_timesteps:
            token_score_col: str = GENERATED_TOKEN_SCORES_COL_TEMPLATE.format(
                token_timestep_i=token_timestep_i
            )
            if row.get(token_score_col) is None:
                continue
            token_score.append(
                cls.combine_token_scores(
                    [get_default(row[token_score_col].get(token), 0.0) for token in tokens],
    ...
    def _entailment_df_to_predictions(
        ...
        *,
        ...
        entailment_df_row_gb: Any = entailment_df.groupby(batch.data_schema.index_col)
    ...
    def _scores_from_row_entailment_df(
        ...
        entailment_logits: Dict[str, float] = dict(
            zip(
                row_entailment_df[PROMPT_TEMPLATE_ID_COL],
                row_entailment_df[ENTAILMENT_LOGIT_COL],
            )
        )
        entailment_logits: np.ndarray = np.array(
            [entailment_logits[label_normalizer(lb)] for lb in labelspace]
        )
...
class FewShotTextMultiLabelClassifier(MultiLabelClassifier, FewShotTextClassifier):
    ...
    def _scores_from_row_entailment_df(
        ...
        entailment_probs: Dict[str, float] = dict(
            zip(
                row_entailment_df[PROMPT_TEMPLATE_ID_COL],
