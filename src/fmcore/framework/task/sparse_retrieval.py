...
from fmcore.constants import DataLayout, DataSplit, MLType, Parallelize
from fmcore.data import (
    FileMetadata,
    ScalableDataFrame,
    ScalableDataFrameRawType,
    ScalableSeries,
    ScalableSeriesRawType,
)
from fmcore.framework import Algorithm, Dataset, load_dataset
from fmcore.framework.task.retrieval import (
...
def _make_bm25_docs_batch(
    docs_df: pd.DataFrame, *, id_col: str, text_col: str, tokenize_text: Callable, store_documents: bool
    ...
    for doc_data_i, doc_data in enumerate(docs_df.to_dict(orient="records")):
        try:
            ...
            bm25_doc: BM25IndexStoreDoc = BM25IndexStoreDoc(
                doc_id=doc_id, doc_len=len(doc_tokens), doc=doc_data
            )
            _bm25_docs_batch.append(
                (
                    bm25_doc,
                    doc_tokens,
                )
    ...
    def get_doc_scores(self, query: str) -> np.ndarray:
        ...
        for token in query_tokens:
            ...
            token_query_score: np.ndarray = self.token_idfs.get(token, 0) * (
                token_freq
                * (self.k1 + 1)
                / (token_freq + self.k1 * (1 - self.b + self.b * doc_lens / self.avgdl))
            )
        ...
        **kwargs,
        ...
        if not isinstance(data, RetrievalCorpus):
            ...
            if isinstance(data, np.ndarray):
                if not data.ndim == 1:
                    raise ValueError(
                        f"Expected input numpy array to have exactly 1 dimensions; found: {data.ndim}"
                    )
            ...
            data: RetrievalCorpus = RetrievalCorpus.of(
                ...
                data_schema=Schema(index_col=INDEX_COL_DEFAULT_NAME, features_schema={text_col: MLType.TEXT}),
            )
...
if _IS_RAY_INSTALLED:
            ...
            for query in queries_batch:
                ...
                if return_actor_id:
                    query_top_k_results: List[Tuple] = [
                        tuple([actor_id] + list(x)) for x in query_top_k_results
                    ]
                if return_perf:
                    query_top_k_results: List[Tuple] = [tuple(list(x) + [perf]) for x in query_top_k_results]
                top_k_batch.append(query_top_k_results)
            return top_k_batch

        def get_docs_by_ids(self, doc_ids: List[str]) -> Dict[str, Optional[BM25IndexStoreDoc]]:
            return {doc_id: self.index_shard.get_doc_by_id(doc_id) for doc_id in doc_ids}
    ...
    class RayBM25RetrievalIndex(BM25RetrievalIndexBase):
        ...
        def _get_index_actor_composite(self, actor_id: str) -> RayActorComposite:
            return only_item(
                [actor_composite for actor_composite in self.index if actor_composite.actor_id == actor_id]
            )
        ...
        def _update_index_from_corpus_gen(
            ...
            **kwargs,
            ...
            with Timer("Index documents from corpus into distributed shards"):
                for corpus_shard_i, corpus_shard in enumerate(corpus_gen):
                    ...
                    index_actor_composite: RayActorComposite = self.index[
                        rnd_idx[corpus_shard_i % num_actors]
                    ]
                ...
                for corpus_shard_i, fut_result in accumulate_iter(
                    index_futs, item_wait=1e-3, iter_wait=10e-3
                ):
            ...
            with Timer("Synchronizing index metadata across distributed shards"):
                ## Next steps:
                ...
                with Timer(">> Fetching metadata from distributed shards"):
                ...
                with Timer(">> Broadcasting metadata to distributed shards"):
                    ...
                    for index_actor_composite in self.index:
                        ...
                        set_token_doc_freqs_futs.append(
                            index_actor.set_token_doc_freqs.remote(combined_token_doc_freqs_obj)
                        )
                    wait(set_index_size_futs)
                    wait(set_corpus_num_tokens_futs)
                    wait(set_token_doc_freqs_futs)
                with Timer(">> Recalculating token_idfs on each shard"):
        ...
        def _retrieve_batch(
            ...
            with Timer(
                f"Retrieving {top_k} results for batch of {num_queries} queries from {num_index_shards} index shards"
            ):
                with Timer(f">> Retrieving {top_k} results for {num_queries} queries from each index shard"):
                    actors_top_k_results: Dict[
                        str, List[List[Tuple[str, BM25IndexStoreDoc, float, Dict]]]
                    ] = {}
                    for index_actor_composite in self.index:
                        index_actor: ray.actor.ActorHandle = index_actor_composite.actor
                        actors_top_k_results[index_actor_composite.actor_id] = (
                            index_actor.get_top_k_batch.remote(
                                queries_batch,
                                top_k=top_k,
                ...
                with Timer(">> Retrieving missing documents to populate cache"):
                    self._populate_docs_cache(actors_top_k_results)

                with Timer(
                    f">> Merging {top_k} results for {num_queries} queries across {num_index_shards} index shards"
                ):
                    batch_ranked_results: List[List[RankedResult]] = []
        ...
        def _populate_docs_cache(
            ...
            if len(actor_to_missing_doc_ids) > 0:
                ...
                for actor_id, actor_doc_id_to_doc in accumulate_iter(
                    actor_to_docs, item_wait=1e-3, iter_wait=1000e-3
                ):
        ...
        def _merge_retrieved_results(
            ...
            with Timer(">>>> Merging retrieved results into top-k RankedResults", silent=True):
                ...
                for k, (doc_actor_id, top_k_doc, top_k_score, perf) in enumerate(query_top_k_results):
                    ...
                    if retrieve_documents and self.params.store_documents:
                        doc: Dict = self._doc_id_to_doc_cache[top_k_doc.doc_id][
                            0
                        ].doc  ## Use the raw document
    ...
    class SparseRetriever(Retriever):
        ...
        def __str__(self) -> str:
            return (
                f"{self.class_name} {f'using {str(self.index)}' if self.index is not None else '(no Index)'}"
            )
        ...
        def predict_step(
            ...
            **kwargs,
            ...
            kwargs["batch_size"]: Optional[int] = get_default(
                kwargs.pop("batch_size", None), self.hyperparams.batch_size
            )
            ranked_results: List[List[RankedResult]] = self.index.retrieve(
                batch, top_k=top_k, retrieve_documents=retrieve_documents, **kwargs
            )
    ...
    class RandomRetriever(Retriever):
        ...
        def _random_retrieve(
            self, queries: Queries, *, top_k: int, retrieve_documents: bool, **kwargs
        ) -> List[List[RankedResult]]:
