...
from fmcore.framework.metric import CountingMetric, Metric, Metrics
...
from fmcore.framework.tracker import DEFAULT_TRACKER_PARAMS, Tracker
from fmcore.util import (
    Alias,
    FractionalBool,
    NeverFailJsonEncoder,
    Parameters,
    Registry,
    String,
    Timer,
...
class Trainer(Parameters, Registry, ABC):
    ...
    custom_definitions: Tuple[Any, ...] = (
        tuple()
    )  ## NOTE! Pydantic complains is you make the typing Callable here.
    ...
    def _set_trainer_params(cls, params: Dict):
        ...
        set_param_from_alias(
            params,
            param="metric_eval_frequency",
            alias=[
                "eval_freq",
                "eval_frequency",
                "metric_eval_freq",
                "eval_epochs",
                "steps_eval_frequency",
                "steps_eval_freq",
                "eval_steps",
                "steps_eval",
            ],
        ...
        custom_definitions: List[Any] = as_list(
            get_default(
        ...
        if isinstance(AlgorithmClass, Algorithm):
            ...
            if params.get("task") is None:
                raise ValueError(
                    "If passing `algorithm` as a string (i.e. class name or alias), "
                    "you must also pass `task`=..."
                )
            params["AlgorithmClass"]: Type[Algorithm] = Algorithm.get_subclass(
                (params["task"], AlgorithmClass)
            )
    ...
    def of(
        ...
        **kwargs,
        ...
        if TrainerClass == Trainer:
            subclasses: List[str] = random_sample(as_list(Trainer.subclasses), n=3, replacement=False)
            raise ValueError(
    ...
    @safe_validate_arguments
    def train(self, datasets: Datasets, metrics: Optional[Metrics] = None, **kwargs):
        ...
        out = self._run_training(datasets=datasets, metrics=metrics, progress_bar=progress_bar, **kwargs)
        return out
    ...
    def _train_single_model(
        ...
        **kwargs,
        ...
        if all_are_not_none(epochs, steps):
            ...
            raise ValueError("Cannot train model without passing `epochs` or `steps` in hyperparams.")
        if epochs is not None:
            ...
            if self.metric_eval_frequency == 1:
                raise ValueError(
                    "Cannot use eval_steps=1 when using `steps`; this will result in calculating metrics "
                    "every single step. Please set `eval_steps` to a higher value."
                )
    ...
    def _train_epoch(
        ...
        **kwargs,
        ...
        if batch_size is None:
            raise ValueError("Cannot train model without passing `batch_size` in hyperparams.")
    ...
    def _train_steps_iter(
        ...
        **kwargs,
        ...
        if self.verbosity >= 2 and progress_bar is None:
            logger(
                f"...training phase of {cur_step_str_with_increment} took {training_timer.time_taken_str}."
            )
        ...
        if any_are_not_none(train_metrics_evaluated, validation_metrics_evaluated, test_metrics_evaluated):
            if self.verbosity >= 2 and progress_bar is None:
                logger(
                    f"...evaluation phase of {cur_step_incremented_str} took {metric_eval_timer.time_taken_str}."
                )
    ...
    def is_final_iter(
        ...
        *,
        ...
        if epochs is not None:
            ...
            return steps == _step_num_after_iter(
                steps=steps, step_num=step_num, steps_increment=steps_increment
            )
        raise ValueError("Exactly one of `epochs` and `steps` must be non-None.")
        ...
        **kwargs,
        ...
        if epochs is not None:
            ...
            log_metrics_prefix: Dict = {
                DataSplit.TRAIN: f"Metric value after {cur_epoch_str}: {DataSplit.TRAIN.name.capitalize()} ",
            ...
            raise ValueError("Exactly one of `epochs` or `steps` must be non-None.")
    ...
    def _train_loop(
        ...
        **kwargs,
        ...
        if all_are_not_none(steps, step_num):  ## step_num is in [1, steps] inclusive
            ...
            kwargs["steps"] = None
        for batch_i, train_step_metrics in enumerate(
            model.train_iter(
                train_dataset,
                batch_size=batch_size,
                dataset_length=train_dataset_length,
            ...
            batches: int = math.ceil(
                train_dataset_length / batch_size
            )  ## Total number of training set batches
        ...
        **kwargs,
        ...
        if not force:
            if all_are_not_none(epoch_num, step_num):
                raise ValueError("Must pass at most one of `epoch_num` and `step_num`; both were passed.")
            elif all_are_none(epoch_num, step_num):
                raise ValueError(
                    "Cannot evaluate model metrics without passing either `epoch_num` or `step_num`."
                )
    ...
    def _log_metrics(
        self, prefix: str, metrics: Optional[List[Optional[Metric]]], logger: Callable, **kwargs
    ):
        if metrics is None:
            return
    ...
    @classmethod
    def _cur_step_str_with_increment(
        cls, step_num: int, steps: int, steps_increment: int, prefix: str = "step"
    ) -> str:
        step_num_str, steps_str, step_num_increment_str = cls._pad_step_strs(
            step_num=step_num,
            steps=steps,
    ...
    @classmethod
    def _pad_step_strs(
        cls, step_num: int, steps: int, steps_increment: Optional[int]
    ...
    def _pad_batch_strs(cls, batch_i: int, batches: int) -> Tuple[str, str]:
        batch_num_str: str = String.pad_zeros(
            batch_i + 1, batches
        )  ## batch_i is in [0, num_batches], inclusive.
