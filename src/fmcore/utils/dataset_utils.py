...
class DatasetUtils:
    ...
    @staticmethod
    def load_and_split_datasets(inputs: Dict[DatasetType, FileMetadata]) -> Dict[DatasetType, pd.DataFrame]:
        """
        ...
        for dataset_type in [DatasetType.TRAIN, DatasetType.VAL, DatasetType.TEST]:
            if dataset_type in inputs:
                data[dataset_type] = DatasetUtils.read_dataset(
                    file_metadata=inputs[dataset_type]
                )
        ...
        if DatasetType.VAL not in data and DatasetType.TEST not in data:
            ...
            train_df, remaining = train_test_split(
                ...
                train_size=DatasetUtils.TRAIN_RATIO, 
                random_state=42
            )
            # Split temp into val and test (50/50 of the 40%, which gives 20/20)
            val_df, test_df = train_test_split(
                remaining, 
                train_size=DatasetUtils.VAL_RATIO_FROM_REMAINING, 
                random_state=42
            )
